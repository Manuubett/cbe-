...
// ── Get the authenticated school uid — source of truth is Firebase Auth ──
function _getSchoolUid(){
  // Priority: 1. Firebase Auth (live, always correct)
  //           2. window._schoolId (set on login, good for current session)
  // NEVER use localStorage alone — it persists across different school logins
  if(typeof firebase !== 'undefined' && firebase.auth && firebase.auth().currentUser){
    return firebase.auth().currentUser.uid;
  }
  return window._schoolId || null;
}

// ── Get cache key per school (keyed by live uid) ──
function _settingsCacheKey(){
  const uid = _getSchoolUid() || 'unknown';
...
async function loadSchoolSettings(db) {
  const uid = _getSchoolUid();
  if(!uid) return null; // can't load without auth
...
function clearSettingsCache() {
  ...
  // Clear old shared key if present (migration cleanup)
  localStorage.removeItem('cbe_school_settings');
}

// ── Clear ALL school settings caches (call on logout) ──
function clearAllSettingsCache() {
  const keys = Object.keys(localStorage).filter(k => k.startsWith('cbe_school_settings_'));
  keys.forEach(k => localStorage.removeItem(k));
  localStorage.removeItem('cbe_school_settings');
  localStorage.removeItem('cbe_current_uid');
}
